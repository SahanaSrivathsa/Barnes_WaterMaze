...
# Map trial number to day
df['_Day'] = ((df['Trial'] - 1) // 6 + 1).clip(upper=4)

df['_TrackFileFormat'] = 'anymaze.csv'
df['Age'] = df['Age']
df['Cohort'] = df['Cohort']
df['_Arena'] = 'Lab/Rtrack/CAS/Arena Files/Cohort' + df[
    'Cohort'].astype(str) + 'Arena.txt'
...
output_file = '/Users/miasponseller/Desktop/CAS_exp_desc.xlsx'
output_df.to_excel(output_file, index=False)

print(f"Saved experiment description file to {output_file}")

...
strategyNames = {'thigmotaxis','circling','random_path','scanning',...
    'chaining','directed_search','corrected_path','direct_path'};
...
middleColor = [.1216, .4667, .7059]; %blue
...
youngData = data1(strcmpi(data1.Age, '6mo'), :);
middleData = data1(strcmpi(data1.Age, '15mo'), :);
oldData   = data1(strcmpi(data1.Age, '23mo'), :);

% Get unique rat IDs 
youngRatIDs = unique(youngData.x_TargetID);
middleRatIDs = unique(middleData.x_TargetID);
oldRatIDs   = unique(oldData.x_TargetID);
totalDays = max(data1.x_Day);

nYoung = numel(youngRatIDs);
nMiddle = numel(middleRatIDs);
...
middleCMap = [linspace(1, middleColor(1), nShades)', ...
             linspace(1, middleColor(2), nShades)', ...
             linspace(1, middleColor(3), nShades)'];

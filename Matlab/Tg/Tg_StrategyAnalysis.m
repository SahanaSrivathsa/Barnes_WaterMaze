...
strat_sheet = readtable(fullfile(base_dir,'Tg_MWM_results_06-17-2025.xlsx'));  % From RTrack, contains Track_ID, Strategy, Age
all_rats_spatial = readtable(fullfile(base_dir,'Tg_AllRats_Spatial_cleaned.csv'));  % From Python script, contains Test_No, Trial_No, Cohort

% Add AgeGroup column (young, middle, old)
all_rats_spatial.AgeGroup = strings(height(all_rats_spatial), 1);
all_rats_spatial.AgeGroup(all_rats_spatial.Age < 9) = 'young';
all_rats_spatial.AgeGroup(all_rats_spatial.Age >= 9 & all_rats_spatial.Age <=14) = 'middle';
all_rats_spatial.AgeGroup(all_rats_spatial.Age >=15) = 'old';
all_rats_spatial.AgeGroup = categorical(all_rats_spatial.AgeGroup);
...
young_rats=unique(all_rats_spatial.Animal(all_rats_spatial.AgeGroup=='young'));
middle_rats=unique(all_rats_spatial.Animal(all_rats_spatial.AgeGroup=='middle'));
old_rats=unique(all_rats_spatial.Animal(all_rats_spatial.AgeGroup=='old'));

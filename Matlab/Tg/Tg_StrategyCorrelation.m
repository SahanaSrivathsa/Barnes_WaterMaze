...
% Create all required figure subdirectories
subDirs = {'CIPL_Strategy', 'StrategyUse', 'Entropy', 'Platform'};
for d = 1:numel(subDirs)
    dirPath = fullfile(fig_dir, subDirs{d});
...
%     dataEntropyOld{d}=entropy_vals(data1.x_Day == uniqueDays(d) & strcmp(data1.Age,'old'));
% end
% plot_bar_sem_WaterMaze(uniqueDays, meanEntropy(:,1), semEntropy(:,1), ...
%     meanEntropy(:,2), semEntropy(:,2), dataEntropyYoung, dataEntropyOld, postHocResults);
% title('Mean Entropy ');
...
for s = 1:numel(strategyNames)
    ...
    for d = 1:numel(uniqueDays)
        dVal = uniqueDays(d);
        dayData = data1(data1.x_Day == dVal, :);
...
for g = 1:nGroups
    ...
    for d = 1:numel(uniqueDays)
        ...
        groupProbDay = groupStrat{g}(data1.x_Day == dVal);
        for r = 1:numel(uniqueRats)
            ratID = uniqueRats{r};
            isCurrentRat = strcmp(dayData.x_TargetID, ratID);
            if ~any(isCurrentRat)
...
for i = 1:nRats
    ...
    for g = 1:nGroups
        ...
        % Mean per day
        for d = 1:nDays
            pdx = ridx & (data1.x_Day==uniqueDays(d));
            ratMeans(i, col) = mean(gp(data1.x_Day(ridx)==uniqueDays(d))); %#ok<*FNDSB>
            col = col + 1;
        end
    end
end
...
uniqueDays  = unique(data1.x_Day);   nDays  = numel(uniqueDays);
rats        = unique(data1.x_TargetID);
nRats       = numel(rats);
...
for g = 1:size(strategyGroups,1)
    ...
    for dIdx = 1:nDays
        ...
        for ageTag = ["young","old"]
            ...
            dayAgeMask = (data1.x_Day == dVal) & strcmp(data1.Age, ageTag);
    ...
    for g = 1:size(strategyGroups,1)
        ...
        for aIdx = 1:2
            ...
            for sIdx = 1:nStrats
                ...
                for d = 1:numel(uniqueDays)
                    mask = (data1.x_Day == uniqueDays(d)) & strcmp(data1.Age, ageTag);

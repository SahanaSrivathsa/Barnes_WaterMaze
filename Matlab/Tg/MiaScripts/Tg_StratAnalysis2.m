...
for i =1:numel(files)
    ...
    bnFields = intersect({'BarnesID','Barnes ID', 'CowenID', 'Cowen ID'}, keyT.Properties.VariableNames);
    if isempty(bnFields)
        warning("Barnes ID or CowenID column not found in %s", fname);
        bnFields = {''};
    end
    ...
    for k = 1:height(keyT)
        ...
        matchIdx = strcmp(spT.Animal, string(BN));% Find matching Animal rows in Spatial
    ...
    % Standardize Animal column to string/cell across all files
    if isnumeric(spT.Animal)
        spT.Animal = cellstr(string(spT.Animal));
    elseif isstring(spT.Animal)
        spT.Animal = cellstr(spT.Animal);
    end

    % Do the same for AllSpatial if it's been populated
    if ~isempty(AllSpatial) && isnumeric(AllSpatial.Animal)
        AllSpatial.Animal = cellstr(string(AllSpatial.Animal));
    end

    % ---- NEW: align columns before concatenating ----
    if isempty(AllSpatial)
        AllSpatial = spT;
    else
        % Add any columns missing from spT (fill with NaN)
        missingInNew = setdiff(AllSpatial.Properties.VariableNames, spT.Properties.VariableNames);
        for c = 1:numel(missingInNew)
            spT.(missingInNew{c}) = NaN(height(spT), 1);
        end
        % Add any columns missing from AllSpatial (fill with NaN)
        missingInAll = setdiff(spT.Properties.VariableNames, AllSpatial.Properties.VariableNames);
        for c = 1:numel(missingInAll)
            AllSpatial.(missingInAll{c}) = NaN(height(AllSpatial), 1);
        end
        % Reorder spT columns to match AllSpatial
...
xticks([1 2])
set(gca,'FontSize',14,'FontWeight','bold','LineWidth',1.5,'Box','off','TickDir','out');xticklabels(['Young' 'Old'])
%% Rest of code
